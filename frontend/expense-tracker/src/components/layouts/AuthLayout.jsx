...
const AuthLayout = ({ children }) => {
  return (
    <div className="flex">
      ...
      <div className="hidden md:block w-[40vw] h-screem bg-violet-50 bg-auth-bg-img bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative">
        ...
        <img
          src={CARD_2}
          className="w-[90%] lg:w=[90%] absolute bottom-10 shadow-lg shadow-blue-400/15"
        ></img>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex gap-6 bg-white p-4 rounded-xl shadow-md shadow-purple-400/10 border border-gray-200/50 z-10">
      ...
      <div>
        ...
        <span className="font-medium text-[20px]">${value}</span>
      </div>
    </div>
  );
};
